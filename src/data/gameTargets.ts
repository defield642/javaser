...
const GAME_TARGET_PROFILES: Array<{
  ...
  {
    match: game => !!game?.packageName && /discord/i.test(game.packageName),
    profile: {
      id: 'discord',
      label: 'Discord',
      targets: [
  ...
  {
    match: game => !!game?.packageName && /ppsspp/i.test(game.packageName),
    profile: {
      id: 'ppsspp',
      label: 'PPSSPP',
...
export const getGameTargetProfile = (
  game: Game | null,
): GameTargetProfile | null => {
  if (!game) return null;
